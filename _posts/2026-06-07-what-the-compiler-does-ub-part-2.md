...
The compiler exploits this: since a shift by an out-of-range amount is UB, the compiler may assume the shift amount is always in range and eliminate bounds checks that follow the shift:

```c
result = x << shift_amount;        // UB if shift_amount >= 32
if (shift_amount >= 32) return 0;  // compiler may eliminate this dead check
```
